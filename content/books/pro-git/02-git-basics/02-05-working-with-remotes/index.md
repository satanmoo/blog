...
- Git 프로젝트에서 ==협업하려면 remote repository를 다루는 법==을 알아야 함
...
- `origin`은 통상 ==자신이 clone한 기준 저장소==

이 출력으로 알 수 있는 것
- 어떤 *remote*에서 변경을 ==pull로 쉽게 가져올 수 있는지==
	- `(fetch)` 표기로 알 수 있음
...
- 이후 `git remote -v`는 `origin`과 `pb` ==두 remote를 모두== 보여줌
...
- 활용 방법
	...
	- 로컬로 받아온 *branch* 에 ==checkout해 들여다보기==
...
- 실행 후엔 해당 *remote*의 ==모든 branch에 대한 reference==를 가지게 됨
...
- ==자동으로 merge하지 않음== — 현재 작업 중인 내용에 영향 없음
	- 적용하려면 ==직접 merge== 해야 함
...
- 즉 *remote*에서 데이터를 가져와 ==현재 branch에 바로 merge==
...
같이 *clone*한 사람이 ==먼저 push한 뒤 내가 push를 시도하면 거절==됨
- 해결 순서
...
- `Local branch configured for 'git pull'` — 어떤 로컬 *branch*가 ==어떤 원격 branch와 merge== 되도록 설정됐는지
...
이 출력으로 알 수 있는 것
- 특정 로컬 *branch*에서 `git push`했을 때 ==어디 branch로== 올라가는지
	- "Local refs configured for 'git push'"
- 서버에 있지만 ==내가 아직 못 가져온 원격 branch== (`new`)
- 서버에서 ==삭제됐는데 내 로컬에 ref만 남은 branch== (`stale`)
- 여러 로컬 *branch* 중 `git pull` 시 ==원격 branch와 자동 merge되도록== 설정된 것들
	- "Local branches configured for 'git pull'"
...
`git remote rename <old> <new>`로 ==remote의 shortname 변경==
- 예: `pb` → `paul`로 이름 바꾸기
...
- 로컬에 저장된 ==원격 branch에 대한 reference==
...
==더 이상 필요 없는 remote를 제거==할 때 `git remote remove <shortname>` 또는 `git remote rm <shortname>` 사용
- 두 명령은 ==동일한 동작==
- 사용 시나리오 예시
	- 서버를 옮긴 경우
	- 더 이상 쓰지 않는 mirror
